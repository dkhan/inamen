  ...
  module TokenCountQuery
    class << self
      def aggregate(db, search_selection: nil, scope: nil, bucket: nil, group: :norm_raw)
        ...
        return lexicon.aggregate(group: group) if lexicon

        aggregate_from_tokens(db, selection, group: group)
      end
      ...
      def aggregate_from_tokens(db, selection, group:)
      ...
      def spellings_for_token_from_tokens(db, token:, selection:, case_sensitive:)
        where_sql, where_params = selection.where_clause
        if case_sensitive
          column = "token_raw"
