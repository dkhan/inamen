...
RSpec.describe Inamen::ChapterReport do
  ...
  describe ".stats_for_chapter" do
    ...
    # New KJV: psalm number line then explicit verse 1 (no implicit verse 1).
    it "reports Psalms 1 without a heading before verse 1" do
      s = described_class.stats_for_chapter(lines, book: "Psalms", chapter: 1)

      expect(s[:verse_count]).to eq(6)
      expect(s[:verse_text_words]).to eq(130)
      expect(s[:combined]).to eq(137)
    end

    # New KJV: psalm number, superscription heading, then explicit verse 1.
    it "reports Psalms 3 with a heading and explicit verse 1" do
      s = described_class.stats_for_chapter(lines, book: "Psalms", chapter: 3)

      expect(s[:verse_count]).to eq(8)
